...
public class SenderViewModelUdp implements Runnable  {
    ...
    public SenderViewModelUdp(String serverIP, JTextArea sendMessageArea,JTextArea consoleArea, int messageSize) {
	...
	public void run() {
        ...
        try {
            	...
            	while(true) {
            		if (NewSocket.tcpconnectionmanager.checkAllClientsNewMessage()) {
                    	
                        consoleArea.append("모든 클라이언트로부터 "+"[" + sentMessageCount + "]의 에코 메시지를 받았으므로 브로드캐스트 중지\n");                            
                        NewSocket.tcpconnectionmanager.AllClientsSetFalse(); //에코메시지 수신여부 초기화 
